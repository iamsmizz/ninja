...
if ( ! defined( 'ET_BUILDER_PRODUCT_VERSION' ) ) {
	...
	define( 'ET_BUILDER_PRODUCT_VERSION', '3.27.2' );
}
...
function et_filter_wp_calculate_image_srcset( $sources, $size_array, $image_src, $image_meta ) {
	...
	if ( ! empty( $image_meta['sizes'] ) ) {
		foreach ( $image_meta['sizes'] as $size_key => $size_data ) {
			...
			$responsive_sources[ $size_data['width'] ] = array(
	...
	// Set the sources data fallback when no responsive images sizes generated yet.
	// Try to put only sizes within desktop and phone sizes.
	if ( ! $responsive_sources ) {
		$screen_sizes = et_screen_sizes();

		foreach ( $sources as $source_key => $source ) {
			if ( $source_key < $screen_sizes['phone'] || $source_key > $screen_sizes['desktop'] ) {
				continue;
			}

			$responsive_sources[ $source_key ] = $source;
		}
	}

	// Add the selected images size to the sources data to force the srcset attributes get rendered
	// when theres only 1 sources data.
	if ( 1 === count( $responsive_sources ) && ! isset( $responsive_sources[ $size_array[0] ] ) ) {
	...
	// Set the sources data fallback as is.
	if ( count( $responsive_sources ) <= 1 ) {
