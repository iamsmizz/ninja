...
class ET_Builder_Module_Helper_MultiViewOptions {
	 ...
	 * @return mixed Value of selected mode.
	 */
	public function get_value( $name, $default_value = '', $mode = 'desktop' ) {
		switch ( $mode ) {
			case 'hover':
	 ...
	 *
	 * @param string       $name           Props name.
	 * @param string|array $selected_modes Selected view modes.
	 * @param string|array $default_value  Default value as fallback data.
	 * @param bool         $distinct       Wether to disticnt the values or not.
	 *
	 * @return array Values of all Breakpoints: desktop, tablet, phone, hover.
	 */
	public function get_values( $name, $selected_modes = false, $default_value = '', $distinct = true ) {
		if ( $selected_modes && is_string( $selected_modes ) ) {
			$selected_modes = false !== strpos( $selected_modes, ',' ) ? explode( ',', $selected_modes ) : array( $selected_modes );
		}

		if ( $selected_modes && ! is_array( $selected_modes ) ) {
			$selected_modes = array( $selected_modes );
		}

		$default_value = $this->normalize_values( $default_value );
		$values        = array();

		foreach ( self::get_modes() as $mode ) {
			if ( $selected_modes && ! in_array( $mode, $selected_modes, true ) ) {
				continue;
			}
			...
			$values[ $mode ] = $this->get_value( $name, $default_value[ $mode ], $mode );
		}

		if ( $distinct ) {
			return $this->distinct_values( $values );
		}
	...
	public function has_value( $name, $value_compare = null, $selected_mode = false, $inherit = false ) {
		...
		$values = $this->get_values( $name, $selected_mode );
		...
		if ( ! $has_value && $selected_mode && 'desktop' !== $selected_mode && $inherit ) {
			$has_value = self::compare_value( $this->get_inherit_value( $name, $selected_mode ), $value_compare );
		}
	...
	public function get_inherit_value( $name, $selected_mode ) {
		$values = $this->get_values( $name, false, null, false );
	...
	protected function populate_data__visibility( $visibility ) {
		...
		foreach ( self::get_modes() as $mode ) {
			...
			foreach ( $visibility as $name => $value_compare ) {
				$data[ $mode ][ $name ] = $this->has_value( $name, $value_compare, $mode, true ) ? 1 : 0;
			}
		}
	...
	protected function filter_value( $raw_value, $args = array() ) {
		...
		if ( $name ) {
			...
			if ( $raw_value && 'content' === $context && 'desktop' !== $mode && in_array( $name, $content_fields, true ) ) {
				...
				// Cleaning up invalid extra <p> tag.
				$raw_value = preg_replace( '/(^<\/p>)(.*)(<p>$)/ius', '$2', trim( $raw_value ) );

				// Cleaning up extra new line.
				$raw_value = preg_replace( '/(^\\n<p>)(.*)(<\/p>\\n$)/ius', '$2', trim( $raw_value ) );
	...
	protected function normalize_values( $values = array() ) {
		...
		foreach ( $normalized as $mode => $value ) {
			if ( preg_match_all( $this->pattern, $value, $matches, PREG_SET_ORDER, 0 ) ) {
				foreach ( $matches as $match ) {
					if ( ! isset( $match[1] ) ) {
						continue;
