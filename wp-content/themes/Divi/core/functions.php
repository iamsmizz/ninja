...
function et_get_attachment_id_by_url( $url ) {
	...
	if ( isset( $id_by_url_cached[ $cache_key ] ) ) {
		return $id_by_url_cached[ $cache_key ];
	}

	// Bail early if URL is invalid.
	if ( ! $url ) {
		$id_by_url_cached[ $cache_key ] = $attachment_id;
	...
	$attachment_id = $wpdb->get_var( $attachments_query );
